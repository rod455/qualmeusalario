// app/(onboarding)/localizacao.tsx
// Tela 2 do onboarding — localização, modelo de trabalho, experiência
// 🆕 Banner ad no rodapé
        ...
        <View style={s.content}>
          ...
          {focused && results.length > 0 && (
            <View style={s.dropdown}>
              {results.map(c => (
                <TouchableOpacity key={`${c.nome}-${c.uf}`} style={s.dropItem} onPress={() => pickCidade(c)}>
                  <Text style={s.dropItemText}>{c.nome}, {c.uf}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
          ...
          {/* Modelo de trabalho */}
          <Text style={[s.label, {marginTop:22}]}>Modelo de trabalho</Text>
          <View style={s.modelRow}>
            {MODELS.map(m => (
              <TouchableOpacity
                key={m}
                style={[s.modelBtn, workModel === m && s.modelBtnActive]}
                onPress={() => setWorkModel(m)}
              >
                <Text style={s.modelIcon}>{MODEL_ICON[m]}</Text>
                <Text style={[s.modelTxt, workModel === m && s.modelTxtActive]}>{MODEL_LABEL[m]}</Text>
              </TouchableOpacity>
            ))}
          </View>

          {/* Experiência */}
          <Text style={[s.label, {marginTop:22}]}>Experiência na área</Text>
          <View style={[s.stepperCard, years === null && s.stepperCardEmpty]}>
            <TouchableOpacity
              style={[s.stepperBtn, displayYears <= 0 && s.stepperBtnOff]}
              onPress={() => changeYears(-1)}
              disabled={displayYears <= 0}
            >
              <Text style={[s.stepperBtnTxt, displayYears <= 0 && s.stepperBtnTxtOff]}>−</Text>
            </TouchableOpacity>
            <TouchableOpacity style={s.stepperCenter} onPress={() => years === null && updateYears(0)}>
              {years !== null ? (
                <Text style={s.stepperNum}>{displayYears}</Text>
              ) : (
            ...
            <TouchableOpacity
              style={[s.stepperBtn, displayYears >= MAX_EXP && s.stepperBtnOff]}
              onPress={() => changeYears(1)}
              disabled={displayYears >= MAX_EXP}
            >
          ...
          {years !== null && (
            <View style={s.sliderWrap}>
              ...
              <View style={s.sliderLabels}>
                <Text style={s.sliderLbl}>0 anos</Text>
                <Text style={s.sliderLbl}>20+ anos</Text>
              </View>
            </View>
          )}
...
const s = StyleSheet.create({
  ...
  inputWrap:     { flexDirection:'row', alignItems:'center', borderWidth:1.5, borderColor:'rgba(255,255,255,0.10)', borderRadius:14, paddingHorizontal:14, paddingVertical:12, backgroundColor:'rgba(255,255,255,0.04)', gap:8 },
  inputFocused:  { borderColor:COLORS.secondary, backgroundColor:'rgba(23,200,232,0.05)' },
  inputIcon:     { fontSize:14 },
  input:         { flex:1, fontSize:15, color:'#fff' },
  dropdown:      { marginTop:4, borderWidth:1, borderColor:'rgba(255,255,255,0.10)', borderRadius:14, backgroundColor:COLORS.surface, overflow:'hidden' },
  dropItem:      { paddingHorizontal:16, paddingVertical:12, borderBottomWidth:0.5, borderBottomColor:'rgba(255,255,255,0.06)' },
  dropItemText:  { fontSize:14, fontWeight:'600', color:'#fff' },
  modelRow:      { flexDirection:'row', gap:8 },
  modelBtn:      { flex:1, flexDirection:'row', alignItems:'center', justifyContent:'center', gap:6, paddingVertical:12, borderWidth:1.5, borderColor:'rgba(255,255,255,0.10)', borderRadius:14, backgroundColor:'rgba(255,255,255,0.03)' },
  modelBtnActive:{ borderColor:COLORS.primary, backgroundColor:'rgba(245,168,32,0.12)' },
  modelIcon:     { fontSize:14 },
  modelTxt:      { fontSize:13, fontWeight:'600', color:'rgba(255,255,255,0.4)' },
  modelTxtActive:{ color:COLORS.primary },
